...
func (c *GitCommand) StashDo(index int, method string) error {
	return c.RunCommandObj(c.NewCmdObjFromStr(fmt.Sprintf("git stash %s stash@{%d}", method, index)))
}

// StashSave save stash
// TODO: before calling this, check if there is anything to save
func (c *GitCommand) StashSave(message string) error {
	return c.RunCommandObj(c.NewCmdObjFromStr("git stash save " + c.OSCommand.Quote(message)))
}
...
func (c *GitCommand) StashSaveStagedChanges(message string) error {
	...
	if err := c.RunCommandObj(c.NewCmdObjFromStr("git stash --keep-index")); err != nil {
		return err
	}
	...
	if err := c.RunCommandObj(c.NewCmdObjFromStr("git stash apply stash@{1}")); err != nil {
		return err
	}
	...
	if err := c.RunCommandObj(c.NewCmdObjFromStr("git stash drop stash@{1}")); err != nil {
		return err
	}
